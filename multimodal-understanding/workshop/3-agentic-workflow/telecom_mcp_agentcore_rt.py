...
def query_to_dataframe(query: str, params: Dict = None) -> pd.DataFrame:
    ...
    try:
        ...
        if df.empty:
            query_logger.info("Query returned no results")
        else:
            query_logger.info(
            ...
            query_logger.debug("Sample data from query", sample_data=df.head().to_dict())
        return df
...
def execute_ddl_statement(statement: str, params: Dict = None) -> bool:
    ...
    ddl_logger = logger.bind(operation="database_ddl_execution")
    
    try:
        ddl_logger.info("Executing DDL statement", 
        ...
        with engine.begin() as conn:
            ...
            ddl_logger.info("DDL statement executed successfully", rows_affected=rows_affected)
            return True
            
    except Exception as e:
        ddl_logger.error("DDL statement execution failed", 
                        error=str(e), 
                        error_type=type(e).__name__,
                        statement_preview=statement[:100] + "..." if len(statement) > 100 else statement)
        return False
...
@tool()
def ask_customer_for_email_or_phone_number() -> str:
    """Ask customer for email or phone number.

    Returns:
        Customer response that should include a phone number or email address
    """
    return get_user_input(prompt="Please provide your phone number or email address so we can help you.")

@tool()
def ask_customer_question() -> str:
    """Ask customer a question that assists in your resolution of their problem

    Returns:
        Customer's answer to your question
    """
...
def get_current_bill(phone: str = "", email: str = "") -> Dict[str, Any]:
    ...
    """
    tool_logger = logger.bind(operation="get_current_bill", phone=phone, email=email)
    tool_logger.info("Retrieving current bill information")
    ...
    if result.get("data"):
        total_amount = result["data"][0].get("TotalAmount", 0)
        invoice_status = result["data"][0].get("InvoiceStatus", "Unknown")
        tool_logger.info(
            "Current bill retrieved",
            total_amount=total_amount,
            invoice_status=invoice_status,
            invoice_count=result.get("count", 0),
        )
    else:
        tool_logger.warning("No current bill found for customer")
...
def check_account_suspension(phone: str = "", email: str = "") -> Dict[str, Any]:
    ...
    tool_logger = logger.bind(
        operation="check_account_suspension", phone=phone, email=email
    )
    tool_logger.info("Checking account suspension status")
    ...
    if result.get("data"):
        account_status = result["data"][0].get("AccountStatus", "Unknown")
        line_status = result["data"][0].get("LineStatus", "Unknown")
        overdue_count = result["data"][0].get("OverdueInvoices", 0)
        overdue_amount = result["data"][0].get("OverdueAmount", 0)

        tool_logger.info(
            "Account suspension check completed",
            account_status=account_status,
            line_status=line_status,
            overdue_invoices=overdue_count,
            overdue_amount=float(overdue_amount) if overdue_amount else 0,
            is_suspended=line_status == "Suspended",
        )
    else:
        tool_logger.info("No suspension data found - account may be in good standing")
...
def add_new_line(plan_name: str, phone: str = "", email: str = "") -> Dict[str, Any]:
    ...
    """
    tool_logger = logger.bind(operation="add_new_line", plan_name=plan_name, phone=phone, email=email)
    tool_logger.info("Adding new line to customer account")
    ...
    if len(df) > 0:
        ...
        if len(customer_results) > 0:
            ...
            tool_logger.info("Customer found", customer_id=customer_id)
...
# create custom model provider to strip out thinking tags
...
agent_logger = logger.bind(operation="agent_initialization")
agent_logger.info("Initializing welcome agent")

welcome_agent = Agent(
    model=model,
    agent_id="welcome_agent",
    system_prompt="""You are a helpful customer support agent for a telecom company. Your job is to assist customers with their service questions and issues.

Follow these instructions:
1. Greet customers warmly and understand their needs from what they've already told you.

2. Get their contact information:
   - If they haven't provided it yet, ask for their phone number or email address
   - You need either one to look up their account and help them

3. Use the available tools to assist them:
   - All tools accept 'phone' or 'email' parameters - pass whichever one the customer provided
   - You can check bills, usage, plans, payment history, add-ons, and more
   - For service changes like adding or canceling lines, use the appropriate tools

4. Be helpful and proactive:
   - Answer their questions clearly
   - Offer relevant information they might need
   - Suggest solutions to their problems

5. If you encounter technical difficulties or can't resolve their issue, route them to the support hotline: 1-800-ANYCO

Important: NEVER disclose internal customer ID numbers or database identifiers to customers.
    """,
    # session_manager=MULTI_AGENT_SESSION_MANAGER,
    conversation_manager=SummarizingConversationManager(),
    callback_handler=None,
    tools=[
        get_complete_customer_profile,
        get_current_bill,
        get_bill_breakdown,
        get_payment_history,
        ...
        get_business_account_summary,
        find_overdue_customers,
        find_high_usage_customers,
        find_loyalty_eligible_customers,
        add_new_line,
        cancel_service_plan,
    ],
)

agent_logger.info("Welcome agent initialized successfully", agent_id="welcome_agent",)

@app.entrypoint
def invoke(user_request):
    invoke_logger = logger.bind(operation="support_agent")
    invoke_logger.debug("Received user request payload: ", user_request=user_request)
    print("user input: ", user_request['input'])
    try:
        results = welcome_agent(user_request['input'])
        invoke_logger.debug("Agent execution completed successfully")
        return results.message
    except Exception as e:
        invoke_logger.error(
            "Agent execution failed", error=str(e), error_type=type(e).__name__
        )
        raise
...
if __name__ == "__main__":
    ...
    startup_logger = logger.bind(operation="server_startup")

    try:
        startup_logger.info(
            "Starting Telecom Support MCP AgentCore Server"
        )
        app.run()

    except Exception as e:
        startup_logger.error(
            "Failed to start server", error=str(e), error_type=type(e).__name__
        )
        raise
