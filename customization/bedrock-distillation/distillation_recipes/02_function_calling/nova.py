...
class NovaHandler(BaseHandler):
    ...
    def _get_base_model(self) -> str:
        """
        Get the base model name for this model.
        
        For custom models, returns the base_model specified in the config.
        For standard models, returns the model name itself.
        """
        # Lazy import to avoid circular dependency
        from bfcl_eval.constants.model_config import MODEL_CONFIG_MAPPING
        
        # Look up the model configuration
        for model_key, model_config in MODEL_CONFIG_MAPPING.items():
            if model_config.model_name == self.model_name:
                if model_config.base_model:
                    return model_config.base_model
                else:
                    return self.model_name
        
        # Fallback: if not found in config, assume it's a standard model
        return self.model_name

    def _get_model_id(self) -> str:
        """
        Get the appropriate model ID for API calls.
        For custom models (ARNs), use the model_name from config directly.
        For standard models, format as us.amazon.{model_name}.
        """
        # Lazy import to avoid circular dependency
        from bfcl_eval.constants.model_config import MODEL_CONFIG_MAPPING
        
        # Look up the actual model_name from config (could be ARN for custom models)
        actual_model_name = self.model_name  # Default fallback
        for model_key, model_config in MODEL_CONFIG_MAPPING.items():
            if model_key == self.model_name:
                actual_model_name = model_config.model_name
                break
        
        if "custom-model-deployment" in actual_model_name or actual_model_name.startswith("arn:aws:bedrock"):
            # Custom model ARN - use as is
            return actual_model_name
        else:
            # Standard Nova model - format with us.amazon prefix
            return f"us.amazon.{actual_model_name.replace('.', ':')}"
    ...
    def _query_FC(self, inference_data: dict):
        ...
        # Debug: Print what we're sending to AWS
        # print(f"[DEBUG] Model name from config: {self.model_name}")
        # print(f"[DEBUG] Model ID for API call: {model_id}")
        # print(f"[DEBUG] Is custom model (ARN): {'arn:aws:bedrock' in model_id}")
        
        # Check if this is a nova-lite-1-5 model that supports reasoning
