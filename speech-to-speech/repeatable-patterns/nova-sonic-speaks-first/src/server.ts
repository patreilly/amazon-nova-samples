...
io.on('connection', (socket) => {
    ...
    try {
        ...
        let session = bedrockClient.createStreamSession(sessionId);
        ...
        socket.on('promptStart', async () => {
            try {
                ...
                // Check if we need a new session (after stop)
                if (!bedrockClient.isSessionActive(sessionId)) {
                    console.log('Creating new session after stop');
                    sessionId = `${socket.id}-${Date.now()}`;
                    session = bedrockClient.createStreamSession(sessionId);
                    bedrockClient.initiateSession(sessionId);
                    
                    // Set up event handlers for new session
                    session.onEvent('contentStart', (data) => {
                        console.log('contentStart:', data);
                        socket.emit('contentStart', data);
                    });
        ...
        socket.on('stopAudio', async () => {
            try {
                ...
                // Emit streamComplete to re-enable start button
                socket.emit('streamComplete');
            }
        });
